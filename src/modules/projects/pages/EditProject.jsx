...
export const EditProject = () => {
  ...
  // Prepopulate states from project lookup
  useEffect(() => {
    if (project) {
      ...
      setSelectedCategories(masterDataService.parseMultiSelect(project.category));
      setSelectedTechnologies(masterDataService.parseMultiSelect(project.technology));
      setSelectedDepartments(masterDataService.parseMultiSelect(project.department));
      ...
      setResourcesList(
        project.resources && project.resources.length > 0
          ? project.resources.map(r => ({
              ...r,
              source: r.source || (r.size === 'Link' || (r.url && !r.url.startsWith('blob:')) ? 'url' : 'upload')
            }))
  ...
  // Auto-generate Slug helper (when title changes, if it is different from original)
  useEffect(() => {
    if (project && title.trim() !== '' && title.trim() !== project.title) {
  ...
  if (!project) {
    return (
      <div className="portal-section" style={{ textAlign: 'center', paddingTop: '160px' }}>
        <h2>Project kit not found in database</h2>
        <Button className="btn-back" onClick={() => navigate(ROUTES.ADMIN_PROJECTS)} style={{ margin: '20px auto' }}>
          Back to Admin Panel
        </Button>
      </div>
    );
  ...
  const handleSaveProject = async (targetStatus) => {
    ...
    if (selectedCategories.length === 0) {
      showToast("Please select at least one project category.", "error");
      return;
    }
    ...
    const projectPayload = {
      ...
      technology: masterDataService.serializeMultiSelect(selectedTechnologies) || 'Arduino',
      category: masterDataService.serializeMultiSelect(selectedCategories),
      department: masterDataService.serializeMultiSelect(selectedDepartments),
      buildTime: '4-6 Hours',
      ...
      components: [],
      specifications: {
        controller: masterDataService.serializeMultiSelect(selectedTechnologies) || 'Arduino Uno',
        sensors: 'Standard sensor configuration',
        communication: masterDataService.serializeMultiSelect(selectedCategories).toLowerCase().includes('iot') ? 'Wi-Fi' : masterDataService.serializeMultiSelect(selectedCategories).toLowerCase().includes('gps') ? 'GSM' : 'None',
        ...
        <form className="terminal-form portal-form-scrollable" onSubmit={(e) => e.preventDefault()}>
          ...
          <div id="sec-basic" className="card-glass" style={{ padding: 'var(--space-4)' }}>
            ...
            <div className="calc-row" style={{ marginBottom: 'var(--space-4)' }}>
              ...
              <div
                onClick={() => document.getElementById('edit-thumb-upload-input').click()}
                style={{
              ...
              >
                {thumbnailPreview ? (
                  ...
                  <>
                    ...
                    <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '4px' }}>Click to upload a new thumbnail</div>
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Any image format · Max 5MB</div>
                  </>
                )}
                {thumbnailPreview && (
                  <div style={{ position: 'absolute', top: '8px', right: '8px', display: 'flex', gap: '6px' }}>
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); document.getElementById('edit-thumb-upload-input').click(); }}
            ...
            <div className="grid-12" style={{ gap: 'var(--space-3)', marginBottom: 'var(--space-3)' }}>
              ...
              <div style={{ gridColumn: 'span 6' }} className="calc-row">
                ...
                <MasterDataMultiSelect
                  ...
                  placeholder="Select Microcontrollers"
                  allowAdd={true}
                />
              </div>
            </div>
