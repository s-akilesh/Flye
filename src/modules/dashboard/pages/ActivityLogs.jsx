...
export const ActivityLogs = () => {
  ...
  // Staged Filters (for drawer applying/resetting)
  const [stagedModuleFilter, setStagedModuleFilter] = useState('all');
  ...
  const handleApplyFilters = () => {
    setModuleFilter(stagedModuleFilter);
  ...
  const sortedLogs = useMemo(() => {
    return [...logs].sort((a, b) => {
      const dateA = a.created_at || a.createdAt || a.time || 0;
      const dateB = b.created_at || b.createdAt || b.time || 0;
      if (sortField === 'date-desc') {
        return new Date(dateB) - new Date(dateA);
      } else if (sortField === 'date-asc') {
        return new Date(dateA) - new Date(dateB);
      } else if (sortField === 'user') {
        const emailA = a.user_email || a.userEmail || (a.user && a.user.email) || '';
        const emailB = b.user_email || b.userEmail || (b.user && b.user.email) || '';
    ...
    >
      ...
      <div className="portal-content" style={{ display: 'flex', flexDirection: 'column', gap: '24px', marginTop: '16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <Card style={{ padding: '20px', border: '1px solid var(--sys-border)', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <span style={{ fontSize: '11px', color: 'var(--txt-muted)', fontWeight: '600', textTransform: 'uppercase' }}>Total Events</span>
              <h3 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--txt-primary)', margin: '4px 0 0 0' }}>{isStatsLoading ? '...' : stats.total}</h3>
            </div>
          </Card>
          <Card style={{ padding: '20px', border: '1px solid var(--sys-border)', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <span style={{ fontSize: '11px', color: 'var(--txt-muted)', fontWeight: '600', textTransform: 'uppercase' }}>Logged Today</span>
              <h3 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--txt-primary)', margin: '4px 0 0 0' }}>{isStatsLoading ? '...' : stats.today}</h3>
            </div>
          </Card>
          <Card style={{ padding: '20px', border: '1px solid var(--sys-border)', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <span style={{ fontSize: '11px', color: 'var(--txt-muted)', fontWeight: '600', textTransform: 'uppercase' }}>Failed Ops</span>
              <h3 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--txt-primary)', margin: '4px 0 0 0' }}>{isStatsLoading ? '...' : stats.failed}</h3>
            </div>
          </Card>
          <Card style={{ padding: '20px', border: '1px solid var(--sys-border)', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <span style={{ fontSize: '11px', color: 'var(--txt-muted)', fontWeight: '600', textTransform: 'uppercase' }}>Active Users</span>
              <h3 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--txt-primary)', margin: '4px 0 0 0' }}>{isStatsLoading ? '...' : stats.activeUsersCount}</h3>
            </div>
          </Card>
        </div>
        ...
        >
          ...
          <div className="admin-filter-panel-grid" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div className="calc-row">
              <label style={{ fontSize: '12px', fontWeight: '600', color: 'var(--text-secondary)' }}>Module</label>
              <select
                className="form-input"
                value={stagedModuleFilter}
                onChange={(e) => setStagedModuleFilter(e.target.value)}
                style={{ width: '100%', padding: '10px 14px' }}
              >
                <option value="all" style={{ color: 'var(--txt-primary)', background: 'var(--sys-surface)' }}>All Modules</option>
                {Object.entries(ACTIVITY_MODULES).map(([key, label]) => (
                  <option key={key} value={label} style={{ color: 'var(--txt-primary)', background: 'var(--sys-surface)' }}>{label}</option>
                ))}
              </select>
            </div>
            ...
            <div className="calc-row">
              ...
              <select
                ...
                onChange={(e) => setStagedStatusFilter(e.target.value)}
                style={{ width: '100%', padding: '10px 14px' }}
              >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '10px 0', textAlign: 'left' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 24px', borderBottom: '1px solid var(--sys-divider)', paddingBottom: '20px', textAlign: 'left' }}>
              <div style={{ textAlign: 'left' }}>
                <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Log ID</span>
                <span style={{ fontSize: '14px', color: 'var(--brand-primary)', fontWeight: 'bold', fontFamily: 'monospace' }}>
                  {`LOG-${selectedLog.id.slice(0, 8).toUpperCase()}`}
                </span>
              </div>
              <div style={{ textAlign: 'left' }}>
                <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Logged Time</span>
                <span style={{ fontSize: '14px', color: 'var(--txt-primary)', fontWeight: '600' }}>
                  {new Date(selectedLog.created_at).toLocaleString('en-IN')}
                </span>
              </div>
              <div style={{ textAlign: 'left' }}>
                <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Action TriggeredBy</span>
                <span style={{ fontSize: '14px', color: 'var(--txt-primary)', fontWeight: '600' }}>
                  {selectedLog.profiles?.full_name || 'System'}
                </span>
              </div>
              <div style={{ textAlign: 'left' }}>
                <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Source / Origin</span>
                <span style={{ fontSize: '12px', color: 'var(--txt-secondary)', fontWeight: 'bold' }}>
                  {selectedLog.source}
                </span>
              </div>
              <div style={{ textAlign: 'left' }}>
                <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Severity Level</span>
                <span style={{
                  fontSize: '11.5px',
                  fontWeight: 'bold',
                  color: selectedLog.severity === 'CRITICAL' || selectedLog.severity === 'ERROR' ? 'var(--status-error)' : selectedLog.severity === 'WARNING' ? 'var(--status-warning)' : 'var(--brand-accent)'
                }}>
                  {selectedLog.severity}
                </span>
              </div>
              <div style={{ textAlign: 'left' }}>
                <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Target Entity</span>
                <span style={{ fontSize: '13px', color: 'var(--txt-secondary)', wordBreak: 'break-all' }}>
            ...
            {/* Description */}
            <div style={{ textAlign: 'left' }}>
              <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '6px' }}>Event Message</span>
              <h4 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--txt-primary)', margin: 0, lineHeight: '1.4' }}>
            ...
            {/* Formatted Metadata */}
            <div style={{ textAlign: 'left' }}>
              <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '6px' }}>Metadata Parameters</span>
              {renderMetadataFields(selectedLog.metadata)}
            </div>

            {/* User Agent */}
            <div style={{ textAlign: 'left' }}>
              <span style={{ display: 'block', fontSize: '11px', color: 'var(--txt-muted)', textTransform: 'uppercase', marginBottom: '6px' }}>User Agent Coordinates</span>
              <div style={{
                background: 'var(--interaction-hover)',
                border: '1px solid var(--sys-border)',
                borderRadius: '6px',
