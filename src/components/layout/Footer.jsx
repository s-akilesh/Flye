...
export const Footer = () => {
  ...
  return (
    <footer className="home-footer" style={{ borderTop: '1px solid rgba(255, 255, 255, 0.08)', padding: '48px 0 24px 0', background: 'rgba(5, 5, 10, 0.6)', backdropFilter: 'blur(10px)', width: '100%', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 var(--page-padding)', width: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', flex: '1 1 200px' }}>
            ...
            <Link to="/projects?filter=departments" style={{ fontSize: '13px', color: 'var(--text-secondary, #9ca3af)', textDecoration: 'none', transition: 'color 0.2s' }}>Departments</Link>
          </div>

          {/* Col 2 */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', flex: '1 1 200px' }}>
            <span style={{ fontSize: '11px', fontWeight: '800', color: 'var(--text-primary, #fff)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Process</span>
            <span onClick={() => handleScrollToSection('how-it-works')} style={{ fontSize: '13px', color: 'var(--text-secondary, #9ca3af)', cursor: 'pointer', transition: 'color 0.2s' }}>How It Works</span>
            <span onClick={() => handleScrollToSection('about')} style={{ fontSize: '13px', color: 'var(--text-secondary, #9ca3af)', cursor: 'pointer', transition: 'color 0.2s' }}>About Us</span>
          </div>
          ...
          {(settings.instagramUrl || settings.youtubeUrl || settings.linkedinUrl || settings.facebookUrl) && (
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              {settings.instagramUrl && (
                <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-muted, #6b7280)', transition: 'color 0.2s' }}>
              ...
              {settings.youtubeUrl && (
                <a href={settings.youtubeUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-muted, #6b7280)', transition: 'color 0.2s' }}>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
              ...
              {settings.linkedinUrl && (
                <a href={settings.linkedinUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-muted, #6b7280)', transition: 'color 0.2s' }}>
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
