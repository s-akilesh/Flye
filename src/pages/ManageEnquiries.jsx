...
export const ManageEnquiries = () => {
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    try {
      ...
      showToast(`Moved ${enq.name || 'Enquiry'} -> ${statusLabels[targetStatus]}`, 'success');
    } catch (err) {
      showToast("❌ Failed to update status.", "error");
    }
  };
    ...
    return (
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: '6px', marginTop: '2px' }}>
          <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{enq.mobile}</span>
